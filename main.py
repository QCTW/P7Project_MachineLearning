...
def expends(X_new, F_new, X_larger, F_larger):
	pass
...
def single_predicte_forest(input_txt, tf_idf, cfs):
	...
	X_top_1000_bigram = expends(X_top_vocab, F_top_vocab, X_top_1000_bigram, F_top_1000_bigram)
	X_capital_count = count_all_capital_words(input_txt)
...
if(console_mode):
	...
	while 1:
		line = input("[*] Please input some text (more than 10 words):\n")
		if line == 'exit':
			break
		elif len(line) <= 10:
			print("[!] Your text is too short (<10), please try again.\n")
			continue
		else:
