...
class Classify:
	def __init__(self):
		self.classifiers = [	(NearestCentroid(), "Nearest Centroid (aka Rocchio)"),
		( MultinomialNB(alpha=.01), "Naive Bayes"),
		( RidgeClassifier(tol=1e-2, solver="auto"), "Ridge Classifier"), 
		( Perceptron(max_iter=50, tol=None), "Perceptron"),
		( PassiveAggressiveClassifier(max_iter=50, tol=None), "Passive-Aggressive"), 
		( KNeighborsClassifier(n_neighbors=5), "K Neighbors"),
	...
	def get_perceptron_clf(self):
		return self.classifiers[3][0]
	def get_passive_aggr_clf(self):
		return self.classifiers[4][0]
	def get_k_neighbors_clf(self):
		return self.classifiers[5][0]
	def get_rand_forest_clf(self):
		return self.classifiers[6][0]
